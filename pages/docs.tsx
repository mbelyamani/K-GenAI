...
const DocsPage: NextPage<Props> = ({ children, meta: pageMeta }: Props) => {
  ...
  const isLoggedIn = localStorage.getItem("isLoggedIn")!=null && localStorage.getItem("isLoggedIn")=="true";
  if ( isLoggedIn ) {
    // router.push('/home');
   } else {
    router.push('/login');
  ...
  return (
    <>
      <MetaTags
        title="Maqasid Answerbot"
        description="Maqasid Sepecific Knowledge Base trained on Maqasid institute material"
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-2 mx-auto">
        ...
        <main className="flex flex-col items-center justify-center flex-1 w-full min-h-screen px-4 py-2 mx-auto mt-12 text-center sm:mt-20">
          <h1 className="max-w-xl text-2xl font-bold sm:text-4xl">
            Ask me anything<sup>*</sup>  about Maqasid Methodology of MI!
          </h1>
          <div className="w-full max-w-xl">
            <textarea
              ...
              rows={4}
              className="w-full p-2 my-5 border rounded-md shadow-md bg-neutral border-neutral-focus text-white"
